...
public abstract class Querier<T> {
	 ...
	 * @param transactor what to do with the data that the cursor points to
	 * @param outputs outputs generated by transactor
	 * @return if eventCursor is null, then return false, otherwise return true
	 */
	protected boolean traverseCursor(Cursor cursor, TraverseTransactor<T> transactor, List<T> outputs) {
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				do {
					T output = transactor.transact(cursor);
					if (outputs == null) {
						continue;
					}
					outputs.add(output);
